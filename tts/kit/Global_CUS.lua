...
--       tap a unit. Done.
--
-- Data comes from the global CUS_LIBRARY_JSON, generated by
-- CUS_MINI_KIT/build_library.ps1 out of the v0.6 faction files with every
-- packet and trait already resolved inline.
--
-- !! THIS IS THE SLIM BUILD AND IT DOES NOT CONTAIN THAT STRING. !!
-- CUS_LIBRARY_JSON is assigned in `unit_library_v06.lua`, which must be pasted
-- ABOVE this script in the same Global tab. Paste this file on its own and the
-- unit library comes up empty — loadLibrary() guards for it, so you get an
-- empty browser rather than an error, which is the harder failure to diagnose.
--
-- If you want one file that needs nothing else, paste `Global_CUS_full.lua`
-- instead: it is this script with the library string appended at the bottom.
-- ===========================================================================
